...
// src/components/dropzone/dropzone.ts
var DROPZONE_ROOT_ATTRIBUTE = "data-sw-dropzone";
var DROPZONE_INPUT_ATTRIBUTE = "data-sw-dropzone-input";
var DROPZONE_UPLOAD_INDICATOR_ATTRIBUTE = "data-sw-dropzone-upload-indicator";
var DROPZONE_LOADING_INDICATOR_ATTRIBUTE = "data-sw-dropzone-loading-indicator";
var DROPZONE_FILES_LIST_ATTRIBUTE = "data-sw-dropzone-files-list";
var DROPZONE_DISABLED_ATTRIBUTE = "data-disabled";
var DROPZONE_DRAG_ACTIVE_ATTRIBUTE = "data-drag-active";
var DROPZONE_HAS_FILES_ATTRIBUTE = "data-has-files";
var DROPZONE_IS_UPLOADING_ATTRIBUTE = "data-is-uploading";
var FILE_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M14 3v4a1 1 0 0 0 1 1h4" /><path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" /></svg>`;
...
function createDropzone(root, options = {}) {
...
var DropzoneController = class {
  ...
  constructor(root, options) {
    ...
    this.isUploading = options.isUploading ?? readBooleanAttribute(root, DROPZONE_IS_UPLOADING_ATTRIBUTE, false);
  ...
  setupInput() {
    ...
    const inputId = this.root.id && this.root instanceof HTMLLabelElement ? `${this.root.id}-input` : ensureId(this.input, "sw-dropzone-input");
...
function getDropzoneElements(root) {
  return {
...
function getOrCreateInput(root) {
  ...
  const fallback = root.querySelector('input[type="file"]');
...
function queryOwnedElement(root, selector) {
...
function constrainFilesForInput(files, input) {
...
function parseAcceptMatchers(accept) {
  return accept.split(",").map((value) => value.trim().toLowerCase()).filter(Boolean).map((token) => {
    if (token.startsWith(".")) {
      return (file) => file.name.toLowerCase().endsWith(token);
    }
    if (token.endsWith("/*")) {
      const mimePrefix = token.slice(0, -1);
      return (file) => file.type.toLowerCase().startsWith(mimePrefix);
    }
    return (file) => file.type.toLowerCase() === token;
  });
}
...
function applyVisuallyHiddenStyles(input) {
