...
function formatBytes(bytes) {
	...
	let fraction = unitIndex === 0 && Number.isSafeInteger(num) ? 0 : 1;
...
const supportsRegexLookbehind = /* @__PURE__ */ once(() => {
	try {
		return "ab".replaceAll(RegExp("(?<=a)b", "g"), "c") === "ac";
	} catch {
		/* v8 ignore start */
		return !1;
	}
...
function throttle(callback, delay, options) {
	let leading = options?.leading ?? !0, trailing = options?.trailing ?? !0, timeoutId, windowStart = 0;
	return function(...args) {
		clearTimeout(timeoutId);
		let now = Date.now();
		if (now - windowStart >= delay && (windowStart = now, leading)) {
			callback.apply(this, args);
			return;
		}
		trailing && (timeoutId = setTimeout(() => {
			windowStart = Date.now(), callback.apply(this, args);
